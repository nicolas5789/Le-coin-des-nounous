...
<div id="profil_nounou">
	<div id="infos_nounou">	
		<h1>Profil de <?= htmlspecialchars($nounou->pseudo()); ?></h1>
		<p>Expérience : <?= htmlspecialchars($nounou->experience()); ?> an(s)  </p>
		<p>Place(s) disponible(s) :  <?= htmlspecialchars($nounou->place_dispo()); ?>    </p>
		<p>Ville de résidence :  <span id="address"><?= htmlspecialchars($nounou->ville()); ?></span>   </p>
...
<div id="contact_avis_nous">
	<?php if(isset($_SESSION['profil']) && $_SESSION['profil'] == "parent") { ?>
			...
			<div id="forms_avis">
				<?php if(isset($_SESSION['avis']) && $_SESSION['avis'] == "clear") { ?>
					<div id="set_avis_nounou">
						...
						<div class="formulaire_avis">
			    			...
			    			<form action="index.php?action=addAvis&amp;id=<?= htmlspecialchars($nounou->id()); ?>)" method="POST">
				...
				<?php } else { ?>
					<div>
						...
						<div class="formulaire_avis" >
							<?php if(isset($_SESSION['notice_avis'])){echo $_SESSION['notice_avis'];} ?>
			    			<form action="index.php?action=updateAvis&amp;id=<?= htmlspecialchars($nounou->id()); ?>)" method="POST">
		...
		<div class="formulaire_profil" id="form_mail_nounou">
		    <div class="form-area">  
		        <form action="index.php?action=mailToNounou&amp;idNounou=<?= htmlspecialchars($nounou->id()); ?>" method="POST">
                    <h3>Contacter la nounou</h3>
    				<div class="form-group">
						<label for="email_nounou">Email de <?= htmlspecialchars($nounou->pseudo()); ?> : <input class="form-control" type="text" name="email_nounou" value="<?= htmlspecialchars($nounou->email()); ?>" required readonly></label>
					</div>
					<div class="form-group">
						<label for="pseudo_parent">Votre Pseudo <input class="form-control" type="text" name="pseudo_parent" value="<?= htmlspecialchars($parent->pseudo()); ?>" required readonly></label>
					</div>
					<div class="form-group">
						<label for="email_parent">Votre Email <input class="form-control" type="text" name="email_parent" value="<?= htmlspecialchars($parent->email()); ?>" required readonly></label>
					</div>
                    <div class="form-group">
